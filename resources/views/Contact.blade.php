<x-app-layout>
    <main class="mt-6 sm:mx-10 mx-4">
        ...
        <article class="grid md:grid-cols-2 gap-8 mb-20">
            ...
            <div
                class="border-2 border-white grid justify-center md:text-4xl md:p-10 p-4 text-2xl  rounded-3xl text-center">
                ...
                <div class="flex items-center justify-center gap-4 mb-20">
                    <a href="{{$user?->telegram}}" class="md:h-16 h-10 aspect-square"><img
                            src="{{ asset('assets/telegram.png') }}"
                            alt=""></a>
                    <a href="{{$user?->whatsapp}}" class="md:h-16 h-10 aspect-square"><img
                            src="{{ asset('assets/whatsapp.png') }}"
                            alt=""></a>
                </div>
            ...
            <div class="w-full grid">
                @if (session('message'))
                    <div class="bg-green-500 text-white p-4 rounded-lg mb-6 text-center h-fit">
                        {{ session('message') }}
                    </div>
                @endif
                <form action="{{route('contact.store')}}" method="post">
                    ...
                    <div class="space-y-6 flex justify-center items-center flex-col">
                        <section class="grid md:grid-cols-2 md:gap-4 gap-6 w-full">
                            ...
                            <input placeholder="Name" class="border-2 h-12 border-white bg-black rounded-2xl"
                                   type="text"
                                   name="name" id="name">
                        ...
                        <textarea placeholder="Message" class="border-2 w-full border-white bg-black rounded-2xl"
                                  name="message" id="message"
                                  cols="30" rows="10"></textarea>

                        <button type="submit"
                                class="bg-snapchat-yellow text-black rounded-2xl px-16 py-10 text-3xl font-bold text-center mx-auto mt-14">
                            Send
                        </button>
                    </div>
                </form>
